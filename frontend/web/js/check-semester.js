	...
	$.ajax({
		...
		success: function(result){
			...
			//else if($('.help-block').text() == "") { // nếu người dùng nhập đúng captcha , thì div báo lỗi sẽ trống
				...
				table += '<th>NƠI SINH</th>';
				table += '<th>GIỚI TÍNH</th>';
				if (semester != 'all')
					table += '<th>ĐỢT TỐT NGHIỆP</th>';
				table += '<th>CHI TIẾT</th>';
				table += '</tr>';
				table += '</thead>';
				...
				$.each(result.allStudentInSemester, function(index, item) {
					...
					table += '<td class="align-middle">' + item[4] + '</td>';
					if(item[5] == '1')
						table += '<td class="align-middle"> Nam </td>';
					else
						table += '<td class="align-middle"> Nữ </td>';
					if (semester != 'all'){
						table += '<td class="align-middle">' + item[6] + '</td>';
						table += '<td><a class="btn btn-outline-primary" id = "detail" href="javascript:detailStudent(' + "'" + item[0] + "', '" + item[7] + "'" + ');">Xem chi tiết</a></td>';
					}
					else
						table += '<td><a class="btn btn-outline-primary" id = "detail" href="javascript:studentAllSemester(' + "'" + item[0] + "'" +  ');">Xem chi tiết</a></td>';
